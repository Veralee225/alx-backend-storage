...
1. first line: `x logs` where `x` is the number of documents in this collection
2. second line: `Methods`:
3. 5 lines with the number of documents with the `method` = `["GET", "POST", "PUT", "PATCH", "DELETE"]` in this order (see example below - warning: it’s a tabulation before each line)
4. one line with the number of documents with:
